...
Vector3 Quaternion::ToEuler() const {
	...
	if (t > 0.4999) {
		euler.z = LIW::Maths::RadiansToDegrees(LIW::Maths::PI / 2.0f);
		euler.y = LIW::Maths::RadiansToDegrees(2.0f * atan2(x, w));
		euler.x = 0.0f;
	...
	euler.z = LIW::Maths::RadiansToDegrees(asin(2 * t));
	euler.y = LIW::Maths::RadiansToDegrees(atan2(2 * y*w - 2 * x*z, 1.0f - 2 * sqy - 2 * sqz));
	euler.x = LIW::Maths::RadiansToDegrees(atan2(2 * x*w - 2 * y*z, 1.0f - 2 * sqx - 2.0f*sqz));
...
Quaternion Quaternion::EulerAnglesToQuaternion(float roll, float yaw, float pitch) {
	float cos1 = (float)cos(LIW::Maths::DegreesToRadians(yaw   * 0.5f));
	float cos2 = (float)cos(LIW::Maths::DegreesToRadians(pitch * 0.5f));
	float cos3 = (float)cos(LIW::Maths::DegreesToRadians(roll  * 0.5f));

	float sin1 = (float)sin(LIW::Maths::DegreesToRadians(yaw   * 0.5f));
	float sin2 = (float)sin(LIW::Maths::DegreesToRadians(pitch * 0.5f));
	float sin3 = (float)sin(LIW::Maths::DegreesToRadians(roll  * 0.5f));
...
Quaternion Quaternion::AxisAngleToQuaterion(const Vector3& vector, float degrees) {
	float theta		= (float)LIW::Maths::DegreesToRadians(degrees);
