...
int TestGame0::Initialise()
{
	...
	for (int i = 0; i < TEST_TEXTURE_COUNT; i++) {
		...
		materialTest.m_handleShaderProgram = m_shaderProgramForward;
	}
	...
	float w = (float)TestGlobal::s_rendererForward->GetWidth();
	float h = (float)TestGlobal::s_rendererForward->GetHeight();
...
int TestGame0::Update(LIWPointer<LIWFrameData, LIWMem_Frame> frameData, LIWFiberWorkerPointer thisFiber)
{
	using namespace LIW;
	...
	ptrTT_OGLForwardRender->m_renderer = TestGlobal::s_rendererForward;
