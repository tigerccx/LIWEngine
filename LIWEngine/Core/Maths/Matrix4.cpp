...
Matrix4 Matrix4::Perspective(float znear, float zfar, float aspect, float fov) {
	...
	const float h = 1.0f / tan(fov * LIW::Maths::PI_OVER_360);
...
Matrix4 Matrix4::PerspectiveX(float znear, float zfar, float aspect, float fovX)
{
	Matrix4 m;

	const float w = 1.0f / tan(fovX * LIW::Maths::PI_OVER_360);
	float neg_depth = znear - zfar;
...
Matrix4 Matrix4::Rotation(float degrees, const Vector3 &inaxis)	 {
	...
	float c = cos((float)LIW::Maths::DegreesToRadians(degrees));
	float s = sin((float)LIW::Maths::DegreesToRadians(degrees));
