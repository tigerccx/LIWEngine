...
Quaternion Quaternion::FromEulerAngles(float degreeX, float degreeY, float degreeZ) {
	//float cosy = (float)cos(DegToRad(yaw * 0.5f));
	//float cosp = (float)cos(DegToRad(pitch * 0.5f));
	//float cosr = (float)cos(DegToRad(roll * 0.5f));

	//float siny = (float)sin(DegToRad(yaw * 0.5f));
	//float sinp = (float)sin(DegToRad(pitch * 0.5f));
	//float sinr = (float)sin(DegToRad(roll * 0.5f));

	//Quaternion q;

	//q.x = (siny * sinp * cosr) + (cosy * cosp * sinr);
	//q.y = (siny * cosp * cosr) + (cosy * sinp * sinr);
	//q.z = (cosy * sinp * cosr) - (siny * cosp * sinr);
