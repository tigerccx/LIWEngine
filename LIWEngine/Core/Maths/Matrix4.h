...
namespace NCL {
	namespace Maths {
		...
		public:
			...
			//Gets the diagonal values (floats 1,5, and 10)
			Vector3 GetDiagonal() const;
			//Sets the diagonal values (floats 1,5, and 10)
			...
			// Creates a perspective matrix, with 'znear' and 'zfar' as the near and
			//far planes, using 'aspect' and 'fovX' as the aspect ratio and horizontal
			//field of vision, respectively.
			static Matrix4 PerspectiveX(float znear, float zfar, float aspect, float fovX);
			...
			inline friend std::ostream& operator<<(std::ostream& o, const Matrix4& m) {
				...
				o << "Mat4(" << std::endl;
				o << m.array[0] << ", " << m.array[4] << ", " << m.array[8] << ", " << m.array[12] << std::endl;
				o << m.array[1] << ", " << m.array[5] << ", " << m.array[9] << ", " << m.array[13] << std::endl;
				o << m.array[2] << ", " << m.array[6] << ", " << m.array[10] << ", " << m.array[14] << std::endl;
				o << m.array[3] << ", " << m.array[7] << ", " << m.array[11] << ", " << m.array[15] << std::endl;
				o << ")" << std::endl;
				return o;
			}
		};
